...
def create_config():
    ...
    parser.add_argument("--repo", help="e.g. user_name/raf", type=str, default="raf-project/raf")
    parser.add_argument(
        "--save-output",
    ...
    parser.add_argument(
        "--command", help="command to run", type=str, default="git rev-parse HEAD | tee stdout.log"
    )
    parser.add_argument(
        "--wait",
    ...
    parser.add_argument(
        "--timeout", help="job timeout in minutes. Default 120 mins", default=120, type=int
    )
...
def fetch_cloud_watch_logs(cloudwatch, log_stream_name, start_timestamp):
    ...
    while True:
        ...
        for event in log_events["events"]:
            ...
            tprint_custom_time(datetime.utcfromtimestamp(last_timestamp / 1000.0), event["message"])
...
def get_job_def(aws_batch, job_def_cfg_file, job_type, image):
    """Get the job definition from the config file and job type. It first uses the config file
    to find the job definition name by the given job type, and then determine the revision
    ...
    if job_type not in job_def_cfg:
        raise ValueError(
            "Job type %s is not specified in the job definition config file %s"
            % (job_type, job_def_cfg_file)
        )
    job_def_name = job_def_cfg[job_type]
    ...
    for job_def in job_defs:
        if "containerProperties" in job_def:
            if job_def["containerProperties"]["image"] == image:
                if not match_revision or match_revision["revision"] < job_def["revision"]:
    ...
    if match_revision is None:
        if latest_revision is None:
            raise RuntimeError(
                "No job definition revision with containerProperties found for %s" % job_def_name
            )
        print(
...
def main():
    ...
    job_def = get_job_def(aws_batch, args.job_def_cfg, args.job_type, args.image)
    wait = args.wait

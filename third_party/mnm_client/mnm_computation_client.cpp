...
std::vector<ComputationClient::ComputationPtr> MNMComputationClient::CompileSunda(
      std::vector<ComputationClient::CompileInstance> instances) {
  std::vector<ComputationPtr> results;
  for (const auto& ins : instances) {
    auto* computation = static_cast<GenericComputationMNM*>(ins.computation.get());
    ...
    IRModule ir_module = IRModule::FromExpr(computation->computation());
    const std::unordered_set<Var, ObjectPtrHash, ObjectPtrEqual>& model_states = computation->model_states();
    std::cout << "Compile: " << std::endl;
    std::cout << ::mnm::ir::AsText(ir_module) << std::endl;
    std::cout << "Alias: " << std::endl;
    for (const auto& kv : computation->alias()) {
    ...
    if (!IsIdentityFunction(func)) {
      ...
      ir_module = mnm::pass::InferType()(ir_module);
      // Later passes take place after FoldConstant, because they introduce dummy constants that should not be folded
      ...
      ir_module = mnm::pass::WrapIdentity()(ir_module);
      ir_module = mnm::pass::InferType()(ir_module);
      Array<Integer> model_states_index;
      const char* env_home = std::getenv("HOME");
      std::string home = std::string(env_home) + "/";
      std::string workspace_name = "workspace_" + std::to_string(compilation_cnt_) + "/";
      std::string init_cmd("cd " + home + "pytorch/torch_mnm/ && rm -rf sunda/" + workspace_name +
        " && mkdir sunda/" + workspace_name + " && cp sunda/model.py sunda/" + workspace_name);
      ExecuteCMD(init_cmd);
      std::string compile_cmd("cd " + home + "pytorch/torch_mnm/sunda/" + workspace_name +
        " && neuron-cc compile --framework MXNET "
        "--meta-module model.py --verbose=info -O2 --pipeline compile SaveTemps "
        "--tensorizer-options=\"--no-fuse-param-to-neff --dump-after-all=penguin --save-weights\" "
        "--enable-experimental-mmpack-opt-level=0 "
        "-o model.neff");
      std::ofstream module_file(home + "pytorch/torch_mnm/sunda/" + workspace_name + "module.json");
      std::ofstream model_states_index_file(home + "pytorch/torch_mnm/sunda/" + workspace_name + "model_states_index.json");
      module_file << serialization::SaveJSON(ir_module);
      for (size_t i = 0; i < func->params.size(); ++i) {
        if (model_states.count(func->params[i])) {
          model_states_index.push_back(i);
        }
      }
      model_states_index_file << serialization::SaveJSON(model_states_index);
      module_file.close();
      model_states_index_file.close();
      ExecuteCMD(compile_cmd);
      // std::ifstream neff_file("sunda/workspace/model.neff");
      // std::string neff = slurp(neff_file);
      results.emplace_back(std::make_shared<MNMComputation>(
        ins.computation,
        ConsumeValue(ins.computation->GetProgramShape()),
        ins.devices,
...
std::vector<ComputationClient::DataPtr>
MNMComputationClient::ExecuteSundaComputation(
  ...
  const auto& mnm_computation = static_cast<const MNMComputation&>(computation);
  auto func = Downcast<Function>(static_cast<GenericComputationMNM*>(mnm_computation.computation())->computation());
  std::vector<Value> values;
  Value ret;
  for (const auto& argument : arguments) {
    values.push_back(static_cast<MNMData*>(argument.get())->handle);
  }
  if (mnm_computation.neff_cnt != -1) {
    ir::Map<ir::String, Value> inputs;
    ir::Map<Integer, Integer> alias;
    const char* env_home = std::getenv("HOME");
    std::string home = std::string(env_home) + "/";
    std::string workspace_name = "workspace_" + std::to_string(mnm_computation.neff_cnt) + "/";
    std::ofstream inputs_file(home + "pytorch/torch_mnm/sunda/" + workspace_name + "inputs.json");
    std::ofstream alias_file(home + "pytorch/torch_mnm/sunda/" + workspace_name + "alias.json");
    std::string exe_cmd("cd " + home + "pytorch/torch_mnm/sunda/ && python3 execute.py --neff_cnt " + std::to_string(mnm_computation.neff_cnt));
    LTC_CHECK_EQ(func->params.size(), values.size());
    for (size_t i = 0; i < values.size(); ++i) {
      inputs.Set(func->params[i]->name_hint(), values[i]);
    }
    inputs_file << serialization::SaveJSON(inputs);
    inputs_file.close();
    for (const auto& kv : mnm_computation.alias) {
      alias.Set(kv.first, kv.second);
    }
    alias_file << serialization::SaveJSON(alias);
    alias_file.close();
    ExecuteCMD(exe_cmd);
    std::ifstream results_file(home + "pytorch/torch_mnm/sunda/" + workspace_name + "results.json");
    std::string results_str = slurp(results_file);
    ret = ir::Downcast<Value>(tvm::LoadJSON(results_str));
  } else {
...
MNMComputationClient::ExecuteComputation(
    const Computation& computation, lazy_tensors::Span<const DataPtr> arguments,
    const std::string& device, const ExecuteComputationOptions& options) {
  return ExecuteSundaComputation(computation, arguments, device, options);
  std::function<std::vector<ComputationClient::DataPtr>(Value)> explode_tuple =
    [&](Value val)->std::vector<ComputationClient::DataPtr> {
    if (const auto* tup = val.as<TupleValueObj>()) {
      std::vector<ComputationClient::DataPtr> ret;

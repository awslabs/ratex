...
class ShapeData {
 public:
  ...
  ShapeData(std::vector<ShapeData> element_shapes)
      : element_type_(PrimitiveType::TUPLE), element_shapes_(std::move(element_shapes)) {
  }

  ShapeData(PrimitiveType element_type, std::vector<int64_t> dimensions,
            std::vector<ShapeData> element_shapes, std::vector<int64_t> minor_to_major)
  ...
  const std::vector<int64_t>& minor_to_major() const {
...
class Data {
 public:
  ...
  Data(std::string device, ShapeData shape) : device_(std::move(device)), shape_(std::move(shape)) {
  }
...
struct TensorSource {
  ...
  TensorSource(ShapeData shape, std::string device, PopulateFn populate_fn)
      : shape(std::move(shape)), device(std::move(device)), populate_fn(std::move(populate_fn)) {
  }

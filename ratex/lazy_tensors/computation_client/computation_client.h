...
class ComputationClient {
 public:
  class Computation {
   public:
    Computation(std::shared_ptr<GenericComputation> computation, ProgramShape program_shape,
    ...
    GenericComputation* computation() const {
      return computation_.get();
    }
  ...
  struct CompileInstance {
    ...
    CompileInstance(std::shared_ptr<GenericComputation> computation, std::string compilation_device,
  ...
  virtual std::vector<DataPtr> TransferToServer(lazy_tensors::Span<const TensorSource> tensors) = 0;

  // Reads the tensor literal values stored at TPU server sites, behind the
  // supplied handles.
  virtual std::vector<Literal> TransferFromServer(lazy_tensors::Span<const DataPtr> handles) = 0;

  // Compiles a set of computations.
  virtual std::vector<ComputationPtr> Compile(std::vector<CompileInstance> instances) = 0;

  // Executes computation with arguments and returns the result.
  // The passed device must match the common device of the arguments Data.
  // If options.explode_tuple is true, the output tuple will be decomposed into
  // its single elements.
  virtual std::vector<DataPtr> ExecuteComputation(const Computation& computation,
  ...
  virtual std::vector<std::vector<DataPtr>> ExecuteReplicated(
      ...
      lazy_tensors::Span<const std::string> devices, const ExecuteReplicatedOptions& options) = 0;
  ...
  virtual std::vector<std::vector<DataPtr>> ExecuteParallel(
      ...
      lazy_tensors::Span<const std::string> devices, const ExecuteParallelOptions& options) = 0;
  ...
  virtual std::vector<DataPtr> ExecuteChained(lazy_tensors::Span<const ExecuteChainedOp> ops,
  ...
  virtual void SetReplicationDevices(std::shared_ptr<std::vector<std::string>> devices) = 0;
  ...
  // be returned.
  std::vector<std::string> GetCompilationDevices(const std::string& device,

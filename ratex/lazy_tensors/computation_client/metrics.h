...
class MetricsArena {
 public:
  ...
  void RegisterCounter(const std::string& name, std::shared_ptr<CounterData>* data);

  void ForEachMetric(const std::function<void(const std::string&, MetricData*)>& metric_func);

  void ForEachCounter(const std::function<void(const std::string&, CounterData*)>& counter_func);
...
class Metric {
 public:
  explicit Metric(std::string name, MetricReprFn repr_fn = MetricFnValue, size_t max_samples = 0);
...
#define LTC_VALUE_METRIC(name, value)                                                      \
  do {                                                                                     \
    static ::lazy_tensors::metrics::Metric* __metric =                                     \
        new ::lazy_tensors::metrics::Metric(name, ::lazy_tensors::metrics::MetricFnValue); \
...
class TimedSection {
 public:
  explicit TimedSection(Metric* metric) : metric_(metric), start_(sys_util::NowNs()) {
  }
...
#define LTC_TIMED(name)                                                             \
  static lazy_tensors::metrics::Metric* timed_metric =                              \
      new lazy_tensors::metrics::Metric(name, lazy_tensors::metrics::MetricFnTime); \

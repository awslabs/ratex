...
import _RATEXC
import ratex.lazy_tensor_core.core.ltc_env_vars as xenv
import ratex.lazy_tensor_core.debug.metrics_saver as ms
import ratex.lazy_tensor_core.utils.utils as xu
import ratex.lazy_tensor_core.utils.keyd_queue as kq

_DEVICES = xu.LazyProperty(lambda: _RATEXC._ltc_get_devices())
...
def _get_device_context(device=None):
    if device is None:
        device = _RATEXC._ltc_get_default_device()
    else:
        device = str(device)
...
class CollectiveContext(object):
    def __init__(self, groups=None):
        self.replica_devcount = _RATEXC._ltc_get_replication_devices_count()
...
def lazy_device(n=None, devkind=None):
    ...
    _RATEXC._ltc_set_default_device(device)
...
def lazy_replication_devices(local_devices):
    ...
    replication_devices = []
    for device in _RATEXC._ltc_get_all_devices():
        xdev = parse_lazy_device(device)
        if not xdev:
...
def unlazy(tensors):
    ...
    """
    _RATEXC._ltc_sync_multi(tensors, devices=[], wait=True)
...
def set_replication(device, devices):
    ...
    if devices:
        ...
        _RATEXC._ltc_set_replication_devices(replication_devices)
        devctx.device_index = devices.index(device)
    else:
        _RATEXC._ltc_set_replication_devices([])
...
def check_view_sharing(obj):
    ...
    def tensor_id(t):
        if is_lazy_tensor(t):
            return _RATEXC._ltc_get_tensor_id(t), "lazy"
        return id(t), "torch"

    def alias_id(t):
        if is_lazy_tensor(t):
            aid = _RATEXC._ltc_get_tensor_view_alias_id(t)
...
def _get_all_reduce_token():
    ...
    if token is None:
        token = _RATEXC._ltc_create_token(devctx.device)
...
def _torch_all_reduce(reduce_type, inputs, group=None):
    ...
    for tensor in inputs:
        ...
        cpu_tensor = _RATEXC._ltc_get_cpu_tensors([tensor])[0]
...
def _host_all_reduce(reduce_type, inputs, cctx, scale=None):
    ...
    _RATEXC._ltc_sync_multi(inputs, devices=[], wait=True, sync_ltc_data=True)
    ...
    if cctx.requires_intercore_reduce:
        ...
        devctx.all_reduce_token = _RATEXC._ltc_all_reduce_inplace(
...
def all_reduce(reduce_type, inputs, scale=1.0, groups=None, cctx=None):
    ...
    if cctx.requires_intercore_reduce:
        ...
        if isinstance(inputs, torch.Tensor):
            result = _RATEXC._ltc_all_reduce(
...
def all_to_all(value, split_dimension, concat_dimension, split_count, groups=None):
    ...
    """
    token, devctx = _get_all_reduce_token()
    result = _RATEXC._ltc_all_to_all(
...
def collective_permute(value, pairs):
    ...
    """
    token, devctx = _get_all_reduce_token()
    result = _RATEXC._ltc_collective_permute(value, token, pairs)
    devctx.all_reduce_token = result[1]
    return result[0]
...
def mark_step():
    ...
    _RATEXC._ltc_step_marker(
        _RATEXC._ltc_get_default_device(),
        [],
...
def wait_device_ops(devices=[]):
    ...
    """
    _RATEXC._ltc_wait_device_ops(devices=devices)
...
def _maybe_convert_to_cpu(data, convert=True):
    def convert_fn(tensors):
        _RATEXC._ltc_sync_multi(tensors, devices=[], wait=True, sync_ltc_data=True)
        if not convert:
            return tensors
        return _RATEXC._ltc_get_cpu_tensors(tensors)
...
def send_cpu_data_to_device(data, device):
    def convert_fn(tensors):
        ...
        return _RATEXC._ltc_tensors_from_aten(tensors, devices)
...
def rendezvous(tag, payload=b"", replicas=[]):
    ...
    """
    return _RATEXC._lazy_rendezvous(get_ordinal(), tag, payload, replicas)
...
def set_rng_state(seed, device=None):
    ...
    if device is None:
        device = _RATEXC._ltc_get_default_device()
    _RATEXC._ltc_set_rng_seed(seed, str(device) if device else "")
...
def get_rng_state(device=None):
    ...
    if device is None:
        device = _RATEXC._ltc_get_default_device()
    return _RATEXC._ltc_get_rng_seed(str(device) if device else "")
...
def get_memory_info(device):
    ...
    Returns:
      ...
      memory in KB) keys.
    """
    return _RATEXC._ltc_memory_info(str(device))

...
ir::Value ApplyViewInfo(ir::Value ir_value, const ViewInfo& view_info) {
  switch (view_info.view_type) {
    case ViewInfo::Type::kSelect:
      return ir::MakeNode<ir::ops::Select>(ir_value, view_info.select->dim, view_info.select->start,
    ...
    default:
      LTC_ERROR() << "Invalid view type: " << lazy_tensors::util::GetEnumValue(view_info.view_type);
  }
}

ir::Value ApplyUpdate(ir::Value ir_value, const Alias::UpdateData& update_data) {
  ...
  for (size_t i = update_data.view_infos.size(); i > 0; --i) {
    ...
    switch (view_info.view_type) {
      case ViewInfo::Type::kSelect:
        result = ir::MakeNode<ir::ops::Unselect>(tmp_values[i - 1], result, view_info.select->dim,
                                                 view_info.select->start, view_info.select->end,
                                                 view_info.select->stride);
        break;
      case ViewInfo::Type::kNarrow:
        result = ir::MakeNode<ir::ops::UpdateSlice>(tmp_values[i - 1], result, view_info.indices);
        break;
      case ViewInfo::Type::kNoOp:
        break;
      case ViewInfo::Type::kPermute:
      ...
      case ViewInfo::Type::kDiagonal:
        result = ir::MakeNode<ir::ops::DiagonalViewUpdate>(
            tmp_values[i - 1], result, view_info.diagonal->offset, view_info.diagonal->dim1,
...
ViewInfo::ViewInfo(Type view_type, lazy_tensors::Shape shape, lazy_tensors::Shape source_shape)
    : view_type(view_type),
...
ViewInfo::ViewInfo(Type view_type, const lazy_tensors::Shape& source_shape, SelectInfo select)
    : view_type(view_type),
      shape(ir::ops::Select::MakeSelectShape(source_shape, select.dim, select.start, select.end,
                                             select.stride)),
...
ViewInfo::ViewInfo(Type view_type, const lazy_tensors::Shape& source_shape, DiagonalInfo diagonal)
    : view_type(view_type),
      shape(ir::ops::Diagonal::MakeDiagonalShape(source_shape, diagonal.offset, diagonal.dim1,
                                                 diagonal.dim2)),
      source_shape(source_shape),
...
View::View(lazy_tensors::Shape shape, std::shared_ptr<Alias> alias, ViewInfo view_info)
...
View::View(lazy_tensors::Shape shape, std::shared_ptr<Alias> alias,
    ...
    : view_infos_(std::move(view_infos)), shape_(std::move(shape)), alias_(std::move(alias)) {
}
...
std::shared_ptr<View> View::CreateSubView(lazy_tensors::Shape shape, ViewInfo view_info) {
  std::vector<ViewInfo> view_infos(view_infos_);
  view_infos.push_back(std::move(view_info));
  return std::make_shared<View>(std::move(shape), alias_, std::move(view_infos));
}

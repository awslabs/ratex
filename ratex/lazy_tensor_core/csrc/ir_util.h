...
class Util {
 public:
  ...
  // detected.
  static std::vector<const Node*> ComputePostOrder(const Node* node, EmissionMap* emap);

  static std::vector<const Node*> ComputePostOrder(lazy_tensors::Span<const Node* const> nodes,
                                                   EmissionMap* emap);

...
lazy_tensors::util::ExceptionCleanup LockDevice(const Device& device) {
  ...
  return lazy_tensors::util::ExceptionCleanup(
      [locker = std::move(locker)](lazy_tensors::util::ExceptionCleanup::StatusType status) {
...
// prevention).
std::vector<lazy_tensors::util::ExceptionCleanup> LockDevices(const std::set<Device>& devices) {
...
class DataCacheArena {
 public:
  struct TensorHasher {
    size_t operator()(const at::Tensor& tensor) const {
      return lazy_tensors::util::HashReduce(lazy_tensors::util::HashCombine(
          lazy_tensors::util::GetEnumValue(tensor.scalar_type()), TensorHash(tensor)));
    };
  };
  ...
  using DataCache = lazy_tensors::util::Cache<at::Tensor, lazy_tensors::client::Data, TensorHasher,
                                              TensorComparer>;

  explicit DataCacheArena(size_t max_cache_size) : max_cache_size_(max_cache_size) {
  }
...
DataCacheArena::DataCache* GetDataCache(const Device& device) {
  static const size_t kMaxCacheSize = lazy_tensors::sys_util::GetEnvInt("DEVDATA_CACHE_SIZE", 128);
...
ir::Value IrValueFromScalar(const at::Scalar& value, at::ScalarType scalar_type,
  ...
  lazy_tensors::ComputationClient::DataPtr device_data = TensorToDataHandle(tensor, device);
...
lazy_tensors::ComputationClient::DataPtr GetDeviceData(const at::Scalar& value,
                                                       at::ScalarType scalar_type,
                                                       const Device& device) {
  // Workaround since at::scalar_tensor doesn't support bfloat16 yet.
  at::Tensor t = at::scalar_tensor(
      value, at::TensorOptions(scalar_type == at::ScalarType::BFloat16 ? at::ScalarType::Float
                                                                       : scalar_type));
...
bool IsSpecialScalar(const at::Scalar& value) {
  static bool no_scalars = lazy_tensors::sys_util::GetEnvBool("NO_SPECIAL_SCALARS", false);
...
class LazyTensor::DeviceContextArena {
 ...
 public:
  ...
  ir::Value GetRngSeed(const Device& device) {
    ...
    if (!devctx->seed_ir_value) {
      devctx->seed_ir_value = IrValueFromScalar(MakeIntScalar(devctx->seed), kSeedType, device);
    }
    ...
    ir::Value k =
        ir::ops::ScalarOp(MakeIntScalar(kSeedMul), MakeLtcPrimitiveType(kSeedType, &device));
    ir::Value b =
        ir::ops::ScalarOp(MakeIntScalar(kSeedAdd), MakeLtcPrimitiveType(kSeedType, &device));
 ...
 private:
  ...
  void ForAllDeviceContexts(const std::function<void(DeviceContext*)>& fn, const Device* device) {
    if (device == nullptr) {
...
LazyTensor::Async::Async(SyncTensorCollection* coll,
...
void LazyTensor::Async::Wait() {
  ...
  for (auto& cleanup : unlocker) {
    const lazy_tensors::util::ExceptionCleanup::StatusType& cleanup_status = cleanup.GetStatus();
...
LazyTensor::LazyTensor(lazy_tensors::ComputationClient::DataPtr handle,
                       c10::optional<at::ScalarType> logical_element_type)
    : data_(std::make_shared<Data>(handle, Device(handle->device()), logical_element_type)) {
}

LazyTensor::LazyTensor(ir::Value ir_value, const Device& device,
                       c10::optional<at::ScalarType> logical_element_type)
    : data_(std::make_shared<Data>(std::move(ir_value), device, logical_element_type)) {
  TryLimitGraphSize();
}

LazyTensor::LazyTensor(std::shared_ptr<View> view, const Device& device,
                       c10::optional<at::ScalarType> logical_element_type)
    : data_(std::make_shared<Data>(std::move(view), device, logical_element_type)) {
}
...
lazy_tensors::Shape LazyTensor::shape_with_layout() const {
  auto tensor_shape = shape();
  return MakeArrayShapeFromDimensions(tensor_shape.get().dimensions(),
                                      tensor_shape.get().dynamic_dimensions(),
                                      tensor_shape.get().element_type(), GetDevice().hw_type);
}

const Device& LazyTensor::GetDevice() const {
  return data()->device;
}
...
std::ptrdiff_t LazyTensor::GetViewAliasId() const {
  return data()->view != nullptr ? reinterpret_cast<std::ptrdiff_t>(data()->view->alias().get())
                                 : 0;
}
...
std::string LazyTensor::DumpBackendComputation(const std::vector<LazyTensor>& tensors) {
  ...
  return !ir_values.empty() ? ir::DumpUtil::ToBackend(ir_values, GetCurrentDevice())
                            : std::string();
}
...
void LazyTensor::SetDataHandle(lazy_tensors::ComputationClient::DataPtr handle, bool sync) {
...
void LazyTensor::SetInPlaceIrValue(ir::Value ir_value) {
  ...
  if (tensor_shape.get().element_type() != ir_value.shape().element_type()) {
    ir_value = ir::MakeNode<ir::ops::Cast>(ir_value, tensor_shape.get().element_type());
  }
...
ir::Value LazyTensor::GetIrValueForTensor(const at::Tensor& tensor, const Device& device) const {
  lazy_tensors::ComputationClient::DataPtr data;
  bool read_only = false;
  if (tensor.dim() == 0 && tensor.numel() == 1) {
    at::Scalar value = tensor.item();
    if (IsSpecialScalar(value)) {
      return ir::ops::ScalarOp(std::move(value),
...
ir::Value LazyTensor::GetIrValueForScalar(const at::Scalar& value, const Device& device) {
  return GetIrValueForScalar(value, MakeLtcPrimitiveType(GetScalarType(value), &device), device);
}
...
ir::Value LazyTensor::GetIrValueForScalar(const at::Scalar& value, const lazy_tensors::Shape& shape,
                                          const Device& device) {
  return GetIrValueForScalar(value, shape.element_type(), shape.dimensions(), device);
}

ir::Value LazyTensor::GetIrValueForScalar(const at::Scalar& value, const lazy_tensors::Shape& shape,
                                          c10::optional<at::ScalarType> logical_element_type,
                                          const Device& device) {
  lazy_tensors::PrimitiveType type = logical_element_type
                                         ? MakeLtcPrimitiveType(*logical_element_type, &device)
                                         : shape.element_type();
  return GetIrValueForScalar(value, type, shape.dimensions(), device);
}

View::IrNode LazyTensor::GetViewUpdate(const std::shared_ptr<View>& view) const {
...
std::shared_ptr<View> LazyTensor::CreateView(ViewInfo view_info) const {
  ...
  ViewInfo this_view_info(ViewInfo::Type::kNoOp, ir_value.shape(), ir_value.shape());
  data()->view = std::make_shared<View>(ir_value.shape(), alias, std::move(this_view_info));
  AssignIrValue(ir::Value());
...
LazyTensor LazyTensor::CreateViewTensor(ViewInfo view_info) const {
  return Create(CreateView(std::move(view_info)), GetDevice(), dtype_optional());
}
...
at::Tensor LazyTensor::ToTensor(bool detached) {
  ...
  if (!tensor_data) {
    ...
    std::vector<at::Tensor> tensors = DataHandlesToTensors({GetDataHandle()}, dtype());
    ...
    tensor = *tensor_data;
    if (detached) {
      if (data()->ir_value || data()->handle != nullptr || data()->view != nullptr) {
...
void LazyTensor::UpdateFromTensorOut(at::Tensor tensor) {
  if (data()->view != nullptr && lazy_tensors::ShapeUtil::ElementsIn(shape()) != tensor.numel()) {
    data()->view = nullptr;
  }
...
void LazyTensor::UpdateFromTensorOut(const LazyTensor& tensor) {
  if (data()->view != nullptr && lazy_tensors::ShapeUtil::ElementsIn(shape()) !=
...
std::vector<lazy_tensors::ComputationClient::DataPtr> LazyTensor::GatherTensorsData(
  ...
  for (size_t i = 0; i < tensors.size(); ++i) {
    ...
    if (it != uid_index_map.end()) {
      ...
      lazy_tensors::ComputationClient::DataPtr handle = tensors[i].CurrentDataHandle();
...
std::vector<at::Tensor> LazyTensor::GetTensorsOpByOp(std::vector<LazyTensor>* tensors) {
...
std::vector<at::Tensor> LazyTensor::GetTensors(std::vector<LazyTensor>* tensors) {
  LTC_VLOG(4) << "Trying to get the value of " << tensors->size() << " tensor(s)";
  static const bool op_by_op = lazy_tensors::sys_util::GetEnvBool("GET_TENSORS_OPBYOP", false);
  return op_by_op ? GetTensorsOpByOp(tensors) : GetTensorsFused(tensors);
}

std::vector<at::Tensor> LazyTensor::GetTensorsFused(std::vector<LazyTensor>* tensors) {
  SyncTensorsConfig config;
  config.force_ltc_data = false;
  ...
  std::vector<lazy_tensors::ComputationClient::DataPtr> tensors_data = GatherTensorsData(
      *tensors, async != nullptr ? async->indices : lazy_tensors::Span<const size_t>(),
      async != nullptr ? async->tensors_data
                       : lazy_tensors::Span<const lazy_tensors::ComputationClient::DataPtr>());
  return FetchTensors(tensors, tensors_data, async != nullptr ? &async->indices : nullptr);
}
...
std::vector<at::Tensor> LazyTensor::FetchTensors(
  ...
  for (size_t i = 0; i < tensors->size(); ++i) {
    if (indices != nullptr && sync_index < indices->size() && i == (*indices)[sync_index]) {
      results.push_back(lazy_tensors::MakeTensorFromComputationData(tensors_data[literals_index],
                                                                    (*tensors)[i].dtype()));
      ++literals_index;
      ++sync_index;
    } else {
...
std::vector<LazyTensor> LazyTensor::CreateTensors(const std::vector<at::Tensor>& tensors,
  ...
  for (size_t i = 0; i < handles.size(); ++i) {
    ltc_tensors.push_back(Create(std::move(handles[i]), tensors[i].scalar_type()));
  }
  return ltc_tensors;
}

ir::Value LazyTensor::CreateTensorNode(lazy_tensors::ComputationClient::DataPtr data,
...
ir::Value LazyTensor::MaybeCastIrValue(ir::Value ir_value, const Device& device,
...
LazyTensor LazyTensor::CreateFrom(ir::Value ir_value, const Device& device) const {
  ir_value = MaybeCastIrValue(std::move(ir_value), device,
                              /*logical_element_type=*/c10::nullopt);
  return Create(std::move(ir_value), device, dtype_optional());
}

LazyTensor LazyTensor::CreateFrom(ir::Value ir_value, at::ScalarType logical_element_type) const {
  ir_value = MaybeCastIrValue(std::move(ir_value), GetDevice(), logical_element_type);
...
LazyTensor LazyTensor::CreateFrom(ir::Value ir_value,
  ...
  ir_value = MaybeCastIrValue(std::move(ir_value), GetDevice(), logical_element_type_opt);
...
LazyTensor LazyTensor::CreateFrom(ir::Value ir_value, const Device& device,
  ...
  ir_value = MaybeCastIrValue(std::move(ir_value), device, logical_element_type);
...
LazyTensor::SyncTensorCollection LazyTensor::CollectSyncTensors(
  ...
  LTC_VLOG(4) << "Waiting on device barrier for device " << coll.device << " done!";
  for (size_t i = 0; i < tensors.size(); ++i) {
    if (tensor_ids.insert(tensors[i].GetUniqueId()).second &&
      ...
      if (ir_value) {
        if (ShouldSyncIrValue(ir_value)) {
          ...
          coll.hash = lazy_tensors::util::HashCombine(coll.hash, ir_value.hash());
  ...
  coll.hash = lazy_tensors::util::MHash(
      coll.hash, lazy_tensors::ComputationClient::Get()->GetResourceDomain(coll.device.ToString()));
  if (!at_tensors.empty()) {
  ...
  LTC_VLOG(4) << "Tensors graph hash " << lazy_tensors::util::HexHash(coll.hash) << " on device "
              << coll.device;
  return coll;
}
...
std::shared_ptr<LazyTensor::Async> LazyTensor::TryRunCachedSync(std::vector<LazyTensor>* tensors,
                                                                SyncTensorCollection* coll,
                                                                PostOrderData* po_data) {
  ComputationCache::TypePtr cached_computation = LookupCachedCompile(*tensors, coll->hash);
  if (cached_computation == nullptr) {
    return nullptr;
  }
  ...
  return ScheduleSyncTensorsGraph(tensors, coll, std::move(po_data->parameters_data),
...
LazyTensor::PostOrderData LazyTensor::RunPostOrder(const std::vector<LazyTensor>& tensors,
  ...
  for (auto node : po_data.post_order) {
    ...
    if (device_data != nullptr) {
      lazy_tensors::client::Data::OpaqueHandle handle = device_data->data()->GetOpaqueHandle();
...
std::vector<ir::Value> LazyTensor::CollectRoots(const std::vector<LazyTensor>& tensors,
                                                lazy_tensors::Span<const size_t> indices) {
...
std::vector<lazy_tensors::ComputationClient::DataPtr> LazyTensor::FetchTensorData(
  ...
  for (auto index : indices) {
    ...
    // completes.
    lazy_tensors::ComputationClient::DataPtr handle = tensor.CurrentDataHandle();
    if (handle == nullptr && config.force_ltc_data) {
      const Device& tensor_device = tensor.GetDevice();
      lazy_tensors::Shape shape = MakeShapeWithDeviceLayout(tensor.shape(), tensor_device.hw_type);
...
std::shared_ptr<LazyTensor::Async> LazyTensor::ScheduleSyncTensorsGraph(
  ...
  std::shared_ptr<Async> async = std::make_shared<Async>(
      coll, std::move(parameters_data), std::move(tensors_data), std::move(cached_computation));
  ...
  auto syncfn = [async, hash = coll->hash]() {
    ...
    try {
      LTC_VLOG(3) << "Executing IR graph hash " << lazy_tensors::util::HexHash(hash)
                  << " on device " << async->device << " ...";
      auto results = lazy_tensors::ComputationClient::Get()->ExecuteComputation(
          *async->cached_computation->computation, async->parameters_data, async->device, options);
      LTC_VLOG(3) << "Executing IR graph hash " << lazy_tensors::util::HexHash(hash)
  ...
  lazy_tensors::env::ScheduleIoClosure(async->mwait.Completer(std::move(syncfn)));
  return async;
}
...
std::shared_ptr<LazyTensor::Async> LazyTensor::ScheduleSyncTensorsGraph(
  ...
  return ScheduleSyncTensorsGraph(coll, std::move(parameters_data), std::move(tensors_data),
...
void LazyTensor::SyncTensorsGraph(std::vector<LazyTensor>* tensors,
                                  ...
                                  bool sync_ltc_data) {
  LTC_VLOG(4) << "Trying to get the value of " << tensors->size() << " tensor(s)";
  static const bool op_by_op = lazy_tensors::sys_util::GetEnvBool("SYNC_TENSORS_OPBYOP", false);
  SyncTensorsConfig config;
...
void LazyTensor::SyncLiveTensorsGraph(const Device* device,
                                      lazy_tensors::Span<const std::string> devices, bool wait) {
  auto tensors = GetLiveTensors(device);
  LTC_VLOG(4) << tensors.size() << " live tensors: devices=(" << absl::StrJoin(devices, ",") << ")";
...
void LazyTensor::WaitDeviceOps(lazy_tensors::Span<const std::string> devices) {
  ...
  if (!devices.empty()) {
    ...
    for (auto& device_str : lazy_tensors::ComputationClient::Get()->GetLocalDevices()) {
      wait_devices.insert(Device(device_str));
    }
  }
...
LazyTensor::OpByOpAsync LazyTensor::SyncTensorsGraphOpByOp(
  ...
  struct Async {
    explicit Async(SyncTensorCollection coll,
                   std::vector<lazy_tensors::ComputationClient::DataPtr> tensors_data,
                   std::vector<ir::Value> roots, lazy_tensors::Span<const std::string> devices)
        : coll(std::move(coll)),
          tensors_data(std::move(tensors_data)),
  ...
  SyncTensorCollection coll = CollectSyncTensors(*tensors, config);
  DebugUtil::SaveTensorsGraphInfo("SyncTensorsGraphOpByOp", *tensors, &coll.indices);

  std::vector<ir::Value> roots = CollectRoots(*tensors, coll.indices);
  auto tensors_data = FetchTensorData(tensors, coll.config, coll.indices);
  auto async =
      std::make_shared<Async>(std::move(coll), std::move(tensors_data), std::move(roots), devices);

  auto syncfn = [async]() -> int {
    try {
      LTC_VLOG(3) << "Executing (OpByOp) IR graph hash "
                  << lazy_tensors::util::HexHash(async->coll.hash) << " on device "
                  << async->coll.device << " ...";
      std::vector<lazy_tensors::ComputationClient::DataPtr> results =
          OpByOpExecutor::Get()->Execute(async->roots, async->coll.device.ToString(),
                                         async->devices);
      LTC_VLOG(3) << "Executing (OpByOp) IR graph hash "
                  << lazy_tensors::util::HexHash(async->coll.hash) << " on device "
...
std::shared_ptr<LazyTensor::Async> LazyTensor::SyncTensorsGraphInternal(
  ...
  coll.hash = lazy_tensors::util::HashCombine(coll.hash,
                                              lazy_tensors::util::Hash(po_data.parameter_sequence));
  LTC_VLOG(4) << "Parameter sequence graph hash " << lazy_tensors::util::HexHash(coll.hash);
  ...
  auto cached_computation =
      std::make_shared<CachedComputation>(std::move(compile_result.computation));
  GetComputationCache()->Add(coll.hash, cached_computation);

  return ScheduleSyncTensorsGraph(tensors, &coll, std::move(compile_result.parameters_data),

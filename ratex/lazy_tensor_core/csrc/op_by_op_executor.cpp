...
const lazy_tensors::Shape& GetParameterShape(const ir::Output& operand,
                                             const lazy_tensors::Shape& input_shape) {
  // See comment in GetOutputIndex() about device data WRT computation outpout
  // shape handling.
  const ir::ops::DeviceData* device_data = ir::ops::DeviceData::Cast(operand.node);
  return device_data != nullptr
             ? input_shape
             : lazy_tensors::ShapeUtil::GetTupleElementShape(input_shape, operand.index);
}

lazy_tensors::hash_t ComputeNodeKey(
    const ir::Node* node, lazy_tensors::Span<const lazy_tensors::Shape* const> input_shapes,
    const lazy_tensors::hash_t& seed) {
  lazy_tensors::hash_t key = seed;
  const auto& operands = node->operands();
  for (size_t i = 0; i < operands.size(); ++i) {
    key = lazy_tensors::util::HashCombine(
        key, lazy_tensors::util::ShapeHash(GetParameterShape(operands[i], *input_shapes[i])));
  }
  key = lazy_tensors::util::HashCombine(key, lazy_tensors::util::ShapeHash(node->shape()));
...
lazy_tensors::hash_t GetNodesKeySeed(const std::string& device,
...
OpByOpExecutor::OpByOpExecutor(size_t compile_cache_size) : compile_cache_(compile_cache_size) {
}

std::vector<lazy_tensors::ComputationClient::ExecuteChainedOp> OpByOpExecutor::BuildOps(
  ...
  std::vector<const ir::Node*> post_order = ir::Util::ComputePostOrder(root_nodes);
  ...
  auto compilation_devices =
      lazy_tensors::ComputationClient::Get()->GetCompilationDevices(device, devices);
  lazy_tensors::hash_t nodes_key_seed = GetNodesKeySeed(device, compilation_devices);
  Device exec_device(device);
  std::vector<lazy_tensors::hash_t> cache_keys;
  std::unordered_map<lazy_tensors::hash_t, std::vector<size_t>, lazy_tensors::util::HashReducer>
      compile_indices;
  std::unordered_map<lazy_tensors::hash_t, size_t, lazy_tensors::util::HashReducer>
      cache_keys_instance;
  ...
  std::vector<lazy_tensors::ComputationClient::CompileInstance> compile_instances;
  std::vector<lazy_tensors::ComputationClient::ExecuteChainedOp> chained_exec_ops(
      post_order.size());
  for (size_t i = 0; i < post_order.size(); ++i) {
    const ir::Node* node = post_order[i];
    lazy_tensors::ComputationClient::ExecuteChainedOp& cxop = chained_exec_ops[i];
    ...
    if (device_data != nullptr) {
      ...
      for (auto& operand : node->operands()) {
        ...
        cxop.inputs.push_back({op_index, GetOutputIndex(device_data_ops[op_index], operand.index)});
        op_input_shapes.push_back(&ops_shapes[op_index]);
      }

      lazy_tensors::hash_t cache_key = ComputeNodeKey(node, op_input_shapes, nodes_key_seed);
      ...
      if (cxop.computation == nullptr) {
        ...
        if (cache_key_indices.size() == 1) {
          ...
          auto computation = BuildNodeComputation(node, op_input_shapes, exec_device);
          lazy_tensors::ProgramShape program_shape = ConsumeValue(computation->GetProgramShape());
          compile_shapes.push_back(
              MakeShapeWithDeviceLayout(program_shape.result(), exec_device.hw_type));
          compile_instances.push_back(
              {std::move(computation), device, compilation_devices, &compile_shapes.back()});
          ops_shapes[i] = compile_shapes.back();
        } else {
          ops_shapes[i] = *compile_instances[cache_keys_instance.at(cache_key)].output_shape;
        }
      } else {
  ...
  if (!compile_instances.empty()) {
    LTC_VLOG(3) << "Compiling " << compile_instances.size() << " computations on device " << device;
    auto computation_ptrs =
        lazy_tensors::ComputationClient::Get()->Compile(std::move(compile_instances));
    LTC_VLOG(3) << "Compiling " << computation_ptrs.size() << " computations on device " << device
                << " done!";
...
std::vector<lazy_tensors::ComputationClient::DataPtr> OpByOpExecutor::Execute(
  ...
  return lazy_tensors::ComputationClient::Get()->ExecuteChained(chained_exec_ops, device);
}
...
OpByOpExecutor::AsyncTask OpByOpExecutor::ExecuteAsync(
  ...
  auto taskfn = [this, roots = std::move(roots_vector), devices = std::move(devices_vector),
                 device]() -> AsyncResult { return Execute(roots, device, devices); };
...
OpByOpExecutor* OpByOpExecutor::Get() {
  ...
  static OpByOpExecutor* split_executor = new OpByOpExecutor(compile_cache_size);

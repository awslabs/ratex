...
void ReplaceLtcTensor(const at::Tensor& tensor, LazyTensor new_ltc_tensor) {
  LTCTensorImpl* impl = dynamic_cast<LTCTensorImpl*>(tensor.unsafeGetTensorImpl());
  LTC_CHECK(impl != nullptr) << "Input tensor is not a lazy tensor: " << tensor.toString();
...
LazyTensor GetOrCreateLtcTensor(const c10::optional<at::Tensor>& tensor, const Device& device) {
  if (!IsDefined(tensor)) {
    return LazyTensor();
  }
  auto xtensor = TryGetLtcTensor(*tensor);
...
std::vector<c10::optional<at::Tensor>> LtcCreateOptTensorList(
  ...
  for (size_t i = 0, defined_pos = 0; i < tensors.size(); ++i) {
    if (to_translate[i]) {
      opt_aten_ltc_tensors[i] = std::move(aten_materialzied_tensors[defined_pos++]);
    }
  }
...
void LtcUpdateTensorsMeta(lazy_tensors::Span<const at::Tensor> dest_ltc_tensors,
                          lazy_tensors::Span<const at::Tensor> source_cpu_tensors,
                          lazy_tensors::Span<const size_t> indices) {
  for (auto index : indices) {
    at::Tensor dest = dest_ltc_tensors.at(index);
...
c10::Device SetCurrentDevice(const c10::Device& device) {
  Device prev_device = torch_lazy_tensors::SetCurrentDevice(AtenDeviceToLtcDevice(device));
...
at::Tensor AtenFromLtcTensor(LazyTensor ltc_tensor) {
  return ltc_tensor.is_null()
             ? at::Tensor()
             : at::Tensor(c10::make_intrusive<LTCTensorImpl>(std::move(ltc_tensor)));
}

std::vector<at::Tensor> AtenFromLtcTensors(lazy_tensors::Span<const LazyTensor> ltc_tensors) {
...
at::Tensor CreateLtcTensor(at::Tensor tensor, const c10::optional<Device>& device) {

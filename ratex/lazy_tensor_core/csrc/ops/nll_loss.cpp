...
NllLoss::NllLoss(const Value& logits, const Value& labels, const absl::optional<Value>& weight,
...
NodePtr NllLoss::Clone(OpList operands) const {
  ...
  return MakeNode<NllLoss>(operands.at(0), operands.at(1), weight, reduction_, ignore_index_);
}

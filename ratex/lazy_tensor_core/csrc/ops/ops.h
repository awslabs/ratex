...
inline NodePtr ScalarOp(const at::Scalar& value, lazy_tensors::PrimitiveType type) {
...
inline NodePtr GenericOp(OpKind op, lazy_tensors::Span<const Value> operands,
  ...
  return MakeNode<Generic>(std::move(op), operands, std::move(shape), num_outputs, hash_seed);
}

inline NodePtr GenericOp(OpKind op, lazy_tensors::Span<const Value> operands,
                         const std::function<lazy_tensors::Shape()>& shape_fn,
                         size_t num_outputs = 1, lazy_tensors::hash_t hash_seed = 0x5a2d296e9) {
  return MakeNode<Generic>(std::move(op), operands, shape_fn, num_outputs, hash_seed);
}
...
inline NodePtr GenericOp(OpKind op, lazy_tensors::Shape shape, size_t num_outputs,
                         lazy_tensors::hash_t hash_seed) {
  return MakeNode<Generic>(std::move(op), std::move(shape), num_outputs, hash_seed);
}

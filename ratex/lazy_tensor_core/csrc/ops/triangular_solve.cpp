...
TriangularSolve::TriangularSolve(const Value& rhs, const Value& lhs, bool left_side, bool lower,
                                 bool transpose, bool unit_diagonal)
    : Node(ir::OpKind(at::aten::triangular_solve), {rhs, lhs},
           /*num_outputs=*/2,
           lazy_tensors::util::MHash(left_side, lower, transpose, unit_diagonal)),
...
NodePtr TriangularSolve::Clone(OpList operands) const {
  return MakeNode<TriangularSolve>(operands.at(0), operands.at(1), left_side_, lower_, transpose_,
                                   unit_diagonal_);
}

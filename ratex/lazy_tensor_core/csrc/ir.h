...
struct Output {
  ...
  Output() = default;
  explicit Output(const Node* node, size_t index = 0) : node(node), index(index) {
  }
...
class Node {
 public:
  ...
  // generates.
  Node(OpKind op, OpList operands, lazy_tensors::Shape shape, size_t num_outputs = 1,
  ...
  Node(OpKind op, OpList operands, const std::function<lazy_tensors::Shape()>& shape_fn,
  ...
  Node(OpKind op, lazy_tensors::Shape shape, size_t num_outputs, lazy_tensors::hash_t hash_seed);
  ...
  lazy_tensors::hash_t node_hash() const {
    return node_hash_;
  }
  ...
  std::shared_ptr<UserMetaData> SetUserMetadata(std::shared_ptr<UserMetaData> user_meta) {
 ...
 private:
  ...
  lazy_tensors::Shape GetOpShape(const std::function<lazy_tensors::Shape()>& shape_fn) const;

  static lazy_tensors::hash_t GetOpHash(OpKind op, const lazy_tensors::Shape& shape,

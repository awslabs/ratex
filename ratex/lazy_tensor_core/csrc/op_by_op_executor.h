...
class OpByOpExecutor {
 public:
  ...
  AsyncTask ExecuteAsync(lazy_tensors::Span<const ir::Value> roots, const std::string& device,
                         lazy_tensors::Span<const std::string> devices);

 private:
  using CompileCache =
      lazy_tensors::util::Cache<lazy_tensors::hash_t, lazy_tensors::ComputationClient::Computation,

...
struct TrackerContext {
  TrackerContext(std::string path, int level) : path(std::move(path)), level(level) {
  }
...
TrackerContext* LoadTrackerContext() {
  std::string fntracker_file = lazy_tensors::sys_util::GetEnvString("LTC_FNTRACKER_FILE", "");
  TrackerContext* tctx = nullptr;
  if (!fntracker_file.empty()) {
    tctx = new TrackerContext(
        std::move(fntracker_file),
        lazy_tensors::sys_util::GetEnvInt("LTC_FNTRACKER_LEVEL", std::numeric_limits<int>::max()));

    std::string fn_list = lazy_tensors::sys_util::GetEnvString("LTC_FNTRACKER_LIST", "");

...
def parse_metrics_report(report, dehumanize=True):
    ...
    server_metric_match_gd = [m.groupdict() for m in re.finditer(_SERVER_METRIC_REGEX, report)]
    for gd in itertools.chain(metric_match_gd, server_metric_match_gd):
        ...
        for k, v in gd.items():
            ...
            full_key = "{}__{}{}{}".format(metric_name, k, "_" if units else "", units)
...
def get_data_points_from_metrics_reports(metrics_reports):
    ...
    for report_index in range(len(metrics_reports)):
        ...
        for metric_name in parsed_report:
            data_points[metric_name][report_index] = parsed_report.get(metric_name, None)
    return data_points
...
def compare_metrics(
    data_points, metrics_report, config={"base_expression": "v <= v_mean + (v_stddev * 2.0)"}
):
    ...
    for k, v in sorted(parsed_report.items()):
        if k not in means_and_stddevs:
            ...
            if eval(expression, None, eval_vars) == False:
                difference_report += (
                    "{} failed its expression check. "
                    "Expression: {}.  Mean: {}.  Stddev: {}.  "
                    "Actual Value: {}\n".format(k, expression, v_mean, v_stddev, v)
                )

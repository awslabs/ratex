...
def server_is_alive():
    return (
        len(
            subprocess.Popen(
                ["pgrep", "-f", XRT_SERVER_REGEX], stdout=subprocess.PIPE
            ).stdout.readline()
        )
        != 0
    )
...
# These needs to be called before the _RATEXC module is loaded.
_setup_default_env()
_setup_grpc()
_setup_ltc_flags()
...
def _prepare_to_exit():
    _RATEXC._prepare_to_exit()
    if int(os.environ.get("PT_LTC_DEBUG", "0")):
        _summarize_fn_tracker()


_RATEXC._initialize_aten_bindings()

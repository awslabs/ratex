...
CanonicalIndexInfo TransposeToFront(at::Tensor base, at::TensorList indices) {
  ...
  if (adjacency_info.contiguous_non_null) {
    return {base, std::move(transposed_indices), lazy_tensors::util::Iota<int64_t>(base_rank),
...
ir::NodePtr IndexFillOp(const ir::Value& buffer, int64_t dim, const ir::Value& index,
...
ir::NodePtr IndexAddOp(const ir::Value& buffer, int64_t dim, const ir::Value& index,
...
ir::NodePtr IndexCopyOp(const ir::Value& buffer, int64_t dim, const ir::Value& index,
                        const ir::Value& source) {
  ir::Value index_rank1 = EnsureRank1(index);

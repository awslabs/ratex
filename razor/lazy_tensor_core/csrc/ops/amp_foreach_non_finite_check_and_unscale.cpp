...
lazy_tensors::Shape NodeOutputShape(const OpList& inputs, const Value& found_inf) {
...
std::vector<Value> GetOperandList(lazy_tensors::Span<const Value> operands, const Value& found_inf,
...
AmpForachNonFiniteCheckAndUnscale::AmpForachNonFiniteCheckAndUnscale(const OpList& inputs,
                                                                     const Value& found_inf,
                                                                     const Value& inv_scale)
    : Node(ir::OpKind(at::aten::_amp_foreach_non_finite_check_and_unscale_),
           GetOperandList(inputs, found_inf, inv_scale), NodeOutputShape(inputs, found_inf),
...
NodePtr AmpForachNonFiniteCheckAndUnscale::Clone(OpList operands) const {
  ...
  return MakeNode<AmpForachNonFiniteCheckAndUnscale>(operand_list, operands[sz], operands[sz + 1]);
}

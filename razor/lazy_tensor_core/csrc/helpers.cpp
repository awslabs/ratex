...
lazy_tensors::int64 Helpers::GetCanonicalDimensionIndex(lazy_tensors::int64 dim,
...
lazy_tensors::int64 Helpers::GetDynamicDimension(const lazy_tensors::Shape& shape) {
...
Helpers::MinMax Helpers::MinMaxValues(lazy_tensors::PrimitiveType type) {
  switch (type) {
    ...
    case lazy_tensors::PrimitiveType::S32:
      return {static_cast<int64_t>(std::numeric_limits<lazy_tensors::int32>::lowest()),
              static_cast<int64_t>(std::numeric_limits<lazy_tensors::int32>::max())};
    case lazy_tensors::PrimitiveType::U32:
      return {static_cast<int64_t>(std::numeric_limits<lazy_tensors::uint32>::lowest()),
              static_cast<int64_t>(std::numeric_limits<lazy_tensors::uint32>::max())};
    case lazy_tensors::PrimitiveType::S64:
      return {static_cast<int64_t>(std::numeric_limits<lazy_tensors::int64>::lowest()),
              static_cast<int64_t>(std::numeric_limits<lazy_tensors::int64>::max())};
    case lazy_tensors::PrimitiveType::U64:
      return {static_cast<int64_t>(std::numeric_limits<lazy_tensors::uint64>::lowest()),
              static_cast<int64_t>(std::numeric_limits<lazy_tensors::uint64>::max())};
    case lazy_tensors::PrimitiveType::F16:
      return {static_cast<float>(std::numeric_limits<lazy_tensors::half>::lowest()),
...
absl::optional<Helpers::DynamicReshapeInfo> Helpers::GetDynamicReshapeInfo(
  ...
  lazy_tensors::int64 input_dynamic_dimension = GetDynamicDimension(input_shape);
...
std::vector<lazy_tensors::int64> Helpers::MakeTransposePermutation(lazy_tensors::int64 dim0,
...
lazy_tensors::PrimitiveType Helpers::PromoteType(lazy_tensors::PrimitiveType type1,
  ...
  lazy_tensors::int64 size1 = lazy_tensors::ShapeUtil::ByteSizeOfPrimitiveType(type1);
  lazy_tensors::int64 size2 = lazy_tensors::ShapeUtil::ByteSizeOfPrimitiveType(type2);
...
std::vector<lazy_tensors::int64> Helpers::GetPromotedShape(
  ...
  for (lazy_tensors::int64 i = 0; i < min_size; ++i) {
    ...
    LTC_CHECK(dim1 == dim2 || dim1 == 1 || dim2 == 1)
        << "(" << absl::StrJoin(shape1_dims, ", ") << ") and (" << absl::StrJoin(shape1_dims, ", ")
        << ")";
    if (dim1 == 0 || dim2 == 0) {

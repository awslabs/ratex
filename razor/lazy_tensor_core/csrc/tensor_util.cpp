...
bool ShouldUse32BitLong() {
  bool use_32bit_long = lazy_tensors::sys_util::GetEnvBool("LTC_USE_32BIT_LONG", false);
  if (use_32bit_long) {
...
lazy_tensors::PrimitiveType LtcTypeFromTensorType(at::ScalarType scalar_type,
  ...
  switch (scalar_type) {
    case at::ScalarType::Double:
      return device.hw_type != DeviceType::TPU ? lazy_tensors::PrimitiveType::F64
...
template <typename S>
lazy_tensors::int64 GetFlatTensorOffset(const S& strides,
                                        const std::vector<lazy_tensors::int64>& indices) {
...
template <typename D, typename S>
void CopyData(D* dest, const S* source, lazy_tensors::int64 n, const CopyDirect&) {
  std::copy(source, source + n, dest);
}

template <typename D, typename S>
void CopyData(D* dest, const S* source, lazy_tensors::int64 n, const CopyCasted&) {
...
template <>
void CopyData<at::BFloat16, lazy_tensors::bfloat16>(at::BFloat16* dest,
                                                    const lazy_tensors::bfloat16* source,
                                                    lazy_tensors::int64 n, const CopyCasted&) {
  CheckedMemcpy<at::BFloat16, lazy_tensors::bfloat16>(dest, source, n);
}
template <>
void CopyData<lazy_tensors::bfloat16, at::BFloat16>(lazy_tensors::bfloat16* dest,
                                                    const at::BFloat16* source,
                                                    lazy_tensors::int64 n, const CopyCasted&) {
  CheckedMemcpy<lazy_tensors::bfloat16, at::BFloat16>(dest, source, n);
}

std::vector<lazy_tensors::int64> GetIterationDimensions(const lazy_tensors::Shape& shape) {
  ...
  std::vector<lazy_tensors::int64> iter_dims =
      lazy_tensors::util::ToVector<lazy_tensors::int64>(shape.layout().minor_to_major());
  size_t index = 0;
  lazy_tensors::int64 scaled_dim_size = kMinorDimScale * shape.dimensions(iter_dims[index]);
...
struct CopyPartition {
  explicit CopyPartition(lazy_tensors::Span<const lazy_tensors::int64> dimensions)
...
std::vector<CopyPartition> CreateCopyPartitions(
  ...
  for (lazy_tensors::int64 i = 0; i < dimensions.size(); ++i) {
    if (i != strided_copy_dimension && (max_dim < 0 || dimensions[i] > dimensions[max_dim])) {
      max_dim = i;
    }
  }

  lazy_tensors::int64 num_elements = lazy_tensors::util::Multiply<lazy_tensors::int64>(dimensions);
  lazy_tensors::int64 max_dim_unit_elements = num_elements / dimensions[max_dim];
  lazy_tensors::int64 max_dim_size = dimensions[max_dim];
  lazy_tensors::int64 part_size =
      std::max<lazy_tensors::int64>(std::max<lazy_tensors::int64>(max_dim_size / max_parts, 1),
                                    kMinThreadElements / max_dim_unit_elements);
  std::vector<CopyPartition> parts;
  lazy_tensors::int64 csize = 0;
  while (csize < max_dim_size) {
    lazy_tensors::int64 n = std::min<lazy_tensors::int64>(part_size, max_dim_size - csize);
...
void SlicedCopy(lazy_tensors::Span<const lazy_tensors::int64> dimensions, const SType* src_data,
                lazy_tensors::Span<const lazy_tensors::int64> src_strides, DType* dest_data,
  ...
  while (n < indices.size()) {
    StridedCopy(dest_data + GetFlatTensorOffset(dest_strides, indices), inner_dest_stride,
                src_data + GetFlatTensorOffset(src_strides, indices), inner_src_stride,
                dimensions[iter_dims.front()]);
...
template <typename SType, typename DType>
void CopyTensors(const void* src_buffer, const lazy_tensors::Shape& src_shape, void* dest_buffer,
                 size_t dest_buffer_size, const lazy_tensors::Shape& dest_shape) {
  LTC_CHECK(lazy_tensors::ShapeUtil::SameDimensions(src_shape, dest_shape))
      << src_shape << " vs. " << dest_shape;

  lazy_tensors::int64 total_elements = lazy_tensors::ShapeUtil::ElementsIn(src_shape);
  ...
  if (src_shape.layout().minor_to_major() == dest_shape.layout().minor_to_major()) {
    CopyData<DType, SType>(
        dest_data, src_data, total_elements,
        typename CopyType < NeedCast<SType>::value || NeedCast<DType>::value > ::type());
    ...
    std::vector<lazy_tensors::int64> src_strides = ComputeShapeStrides(src_shape);
    std::vector<lazy_tensors::int64> dest_strides = ComputeShapeStrides(dest_shape);
    std::vector<lazy_tensors::int64> iter_dims = GetIterationDimensions(dest_shape);
    std::vector<CopyPartition> parts =
    ...
    for (size_t i = 0; i < parts.size(); ++i) {
      auto copy_fn = [&, i]() {
        SlicedCopy<SType, DType>(dest_shape.dimensions(), src_data, src_strides, dest_data,
                                 dest_strides, iter_dims, parts[i]);
      };
...
template <typename SType, typename DType>
void TensorToBuffer(const at::Tensor& tensor, const lazy_tensors::Shape& dest_shape,
                    void* dest_buffer, size_t dest_buffer_size, const Device& device) {
  at::Tensor contiguous_tensor = tensor.contiguous();
  lazy_tensors::Shape src_shape =
      MakeTorchTensorLayout(Helpers::I64List(contiguous_tensor.sizes()), /*dynamic_dimensions=*/{},
                            LtcTypeFromTensorType(contiguous_tensor.type().scalarType(), device));
  CopyTensors<SType, DType>(contiguous_tensor.data_ptr<SType>(), src_shape, dest_buffer,
                            dest_buffer_size, dest_shape);
}

template <typename SType>
void TensorToBufferSType(const at::Tensor& tensor, const lazy_tensors::Shape& dest_shape,
                         void* dest_buffer, size_t dest_buffer_size, const Device& device) {
  switch (dest_shape.element_type()) {
    case lazy_tensors::PrimitiveType::BF16:
      TensorToBuffer<SType, lazy_tensors::bfloat16>(tensor, dest_shape, dest_buffer,
    ...
    case lazy_tensors::PrimitiveType::F64:
      TensorToBuffer<SType, double>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case lazy_tensors::PrimitiveType::PRED:
      TensorToBuffer<SType, bool>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case lazy_tensors::PrimitiveType::U8:
      TensorToBuffer<SType, lazy_tensors::uint8>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                                 device);
      break;
    case lazy_tensors::PrimitiveType::S8:
      TensorToBuffer<SType, lazy_tensors::int8>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                                device);
      break;
    case lazy_tensors::PrimitiveType::S16:
      TensorToBuffer<SType, lazy_tensors::int16>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                                 device);
      break;
    case lazy_tensors::PrimitiveType::U16:
      TensorToBuffer<SType, lazy_tensors::uint16>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                                  device);
      break;
    case lazy_tensors::PrimitiveType::S32:
      TensorToBuffer<SType, lazy_tensors::int32>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                                 device);
      break;
    case lazy_tensors::PrimitiveType::U32:
      TensorToBuffer<SType, lazy_tensors::uint32>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                                  device);
      break;
    case lazy_tensors::PrimitiveType::S64:
      TensorToBuffer<SType, lazy_tensors::int64>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                                 device);
      break;
    case lazy_tensors::PrimitiveType::U64:
      TensorToBuffer<SType, lazy_tensors::uint64>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                                  device);
      break;
    case lazy_tensors::PrimitiveType::C64:
      TensorToBuffer<SType, lazy_tensors::complex64>(tensor, dest_shape, dest_buffer,
                                                     dest_buffer_size, device);
      break;
    case lazy_tensors::PrimitiveType::C128:
      TensorToBuffer<SType, lazy_tensors::complex128>(tensor, dest_shape, dest_buffer,
                                                      dest_buffer_size, device);
      break;
    default:
...
lazy_tensors::ComputationClient::DataPtr TensorToDataHandle(const at::Tensor& tensor,
                                                            ...
                                                            const Device& device) {
  auto populate_fn = [&](const lazy_tensors::ComputationClient::TensorSource& source_tensor,
                         void* dest_buffer, size_t dest_buffer_size) {
    PopulateTensorBuffer(tensor, source_tensor.shape, dest_buffer, dest_buffer_size, device);
  };

  std::vector<lazy_tensors::client::TensorSource> source_tensors;
  source_tensors.emplace_back(lazy_tensors::ToShapeData(shape), device.ToString(),
...
template <typename SType, typename DType>
at::Tensor LiteralToTensor(const lazy_tensors::Literal& literal, at::ScalarType atype) {
  std::vector<int64_t> dimensions =
      lazy_tensors::util::ToVector<int64_t>(literal.shape().dimensions());
  lazy_tensors::Shape torch_shape = MakeTorchTensorLayout(
      literal.shape().dimensions(), /*dynamic_dimensions=*/{}, literal.shape().element_type());
  lazy_tensors::int64 total_elements = lazy_tensors::ShapeUtil::ElementsIn(torch_shape);

  const auto literal_data = literal.data<SType>();
  at::Tensor tensor = at::empty(dimensions, at::TensorOptions(atype));
  CopyTensors<SType, DType>(literal_data.data(), literal.shape(), tensor.data_ptr<DType>(),
...
at::Tensor LiteralToTensorHelper(const lazy_tensors::Literal& literal,
  ...
  switch (dest_element_type) {
    ...
    case at::ScalarType::ComplexFloat:
      return LiteralToTensor<SType, c10::complex<float>>(literal, dest_element_type);
    case at::ScalarType::ComplexDouble:
      return LiteralToTensor<SType, c10::complex<double>>(literal, dest_element_type);
    default:
...
void PopulateTensorBuffer(const at::Tensor& tensor, const lazy_tensors::Shape& dest_shape,
  ...
  switch (tensor.type().scalarType()) {
    case at::ScalarType::Double:
      TensorToBufferSType<double>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case at::ScalarType::Float:
      TensorToBufferSType<float>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case at::ScalarType::BFloat16:
      TensorToBufferSType<at::BFloat16>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case at::ScalarType::Half:
      TensorToBufferSType<at::Half>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case at::ScalarType::Bool:
      TensorToBufferSType<bool>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case at::ScalarType::Byte:
      TensorToBufferSType<uint8_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case at::ScalarType::Char:
      TensorToBufferSType<int8_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case at::ScalarType::Short:
      TensorToBufferSType<int16_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case at::ScalarType::Int:
      TensorToBufferSType<int32_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case at::ScalarType::Long:
      TensorToBufferSType<int64_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case at::ScalarType::ComplexFloat:
      TensorToBufferSType<c10::complex<float>>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                               device);
      break;
    case at::ScalarType::ComplexDouble:
      TensorToBufferSType<c10::complex<double>>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                                device);
      break;
    default:
...
std::vector<lazy_tensors::int64> ComputeShapeStrides(const lazy_tensors::Shape& shape) {
...
at::Tensor MakeTensorFromLiteral(const lazy_tensors::Literal& literal,
  ...
  switch (literal.shape().element_type()) {
    ...
    case lazy_tensors::PrimitiveType::U8:
      return LiteralToTensorHelper<lazy_tensors::uint8>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::S8:
      return LiteralToTensorHelper<lazy_tensors::int8>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::S16:
      return LiteralToTensorHelper<lazy_tensors::int16>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::U16:
      return LiteralToTensorHelper<lazy_tensors::uint16>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::S32:
      return LiteralToTensorHelper<lazy_tensors::int32>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::U32:
      return LiteralToTensorHelper<lazy_tensors::uint32>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::S64:
      return LiteralToTensorHelper<lazy_tensors::int64>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::U64:
      return LiteralToTensorHelper<lazy_tensors::uint64>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::C64:
      return LiteralToTensorHelper<lazy_tensors::complex64>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::C128:
      return LiteralToTensorHelper<lazy_tensors::complex128>(literal, dest_element_type);
    default:
...
lazy_tensors::ComputationClient::DataPtr TensorToDataHandle(const at::Tensor& tensor,
                                                            const Device& device) {
  return TensorToDataHandle(tensor, CreateComputationShapeFromTensor(tensor, &device), device);
}
...
std::vector<lazy_tensors::ComputationClient::DataPtr> CreateTensorsData(
  ...
  for (size_t i = 0; i < tensors.size(); ++i) {
    ...
    lazy_tensors::Shape shape = CreateComputationShapeFromTensor(tensors[i], &device);
    result.push_back(MakeComputationDataFromTensor(tensors[i], shape, devices[i]));
  }
  return result;
}
...
lazy_tensors::Literal GetTensorLiteral(const at::Tensor& tensor, const lazy_tensors::Shape* shape,
  ...
  if (shape == nullptr) {
    ...
    computed_shape =
        MakeTorchTensorLayout(dimensions, /*dynamic_dimensions=*/{},
                              LtcTypeFromTensorType(tensor.type().scalarType(), ltc_device));
    shape = &computed_shape;
  }
  lazy_tensors::Literal literal(*shape);
  PopulateTensorBuffer(tensor, *shape, literal.untyped_data(), literal.size_bytes(), ltc_device);
  return literal;
}
...
std::vector<at::Tensor> DataHandlesToTensors(
  ...
  for (const auto& handle : data_handles) {
    tensors.push_back(lazy_tensors::MakeTensorFromComputationData(handle, dest_element_type));
  }
  return tensors;
}
...
lazy_tensors::hash_t TensorHash(const at::Tensor& tensor) {
  ...
  switch (ctensor.scalar_type()) {
    ...
    case at::ScalarType::BFloat16:
      return lazy_tensors::util::DataHash(ctensor.data_ptr<at::BFloat16>(), size);
    case at::ScalarType::Half:
      return lazy_tensors::util::DataHash(ctensor.data_ptr<at::Half>(), size);
    case at::ScalarType::ComplexFloat:
      return lazy_tensors::util::DataHash(ctensor.data_ptr<c10::complex<float>>(), size);
    case at::ScalarType::ComplexDouble:
      return lazy_tensors::util::DataHash(ctensor.data_ptr<c10::complex<double>>(), size);
    default:
...
lazy_tensors::Shape MakeShapeWithDeviceLayout(const lazy_tensors::Shape& shape,
  ...
  lazy_tensors::ShapeUtil::ForEachMutableSubshape(
      &device_shape, [&](lazy_tensors::Shape* subshape, const lazy_tensors::ShapeIndex&) {
        if (subshape->IsArray()) {
          *subshape =
              MakeArrayShapeFromDimensions(subshape->dimensions(), subshape->dynamic_dimensions(),
...
lazy_tensors::PrimitiveType GetDevicePrimitiveType(lazy_tensors::PrimitiveType type,
  ...
  switch (type) {
    case lazy_tensors::PrimitiveType::F64:
      ...
      return ltc_device.hw_type != DeviceType::TPU ? lazy_tensors::PrimitiveType::F64
                                                   : lazy_tensors::PrimitiveType::F32;
    case lazy_tensors::PrimitiveType::F32:
      if (UseF16()) {
        return lazy_tensors::PrimitiveType::F16;
      }
      return UseBF16() ? lazy_tensors::PrimitiveType::BF16 : lazy_tensors::PrimitiveType::F32;
    case lazy_tensors::PrimitiveType::U16:
      return ltc_device.hw_type != DeviceType::TPU ? lazy_tensors::PrimitiveType::U16
                                                   : lazy_tensors::PrimitiveType::U32;
    case lazy_tensors::PrimitiveType::S16:
      return ltc_device.hw_type != DeviceType::TPU ? lazy_tensors::PrimitiveType::S16
                                                   : lazy_tensors::PrimitiveType::S32;
    case lazy_tensors::PrimitiveType::S64:
      return Use32BitLong() ? lazy_tensors::PrimitiveType::S32 : lazy_tensors::PrimitiveType::S64;
    case lazy_tensors::PrimitiveType::U64:
      return Use32BitLong() ? lazy_tensors::PrimitiveType::U32 : lazy_tensors::PrimitiveType::U64;
    case lazy_tensors::PrimitiveType::C128:
      return ltc_device.hw_type != DeviceType::TPU ? lazy_tensors::PrimitiveType::C128
...
bool RequiresRawTypeCasting(at::ScalarType scalar_type, const Device* device) {
  switch (scalar_type) {
    ...
    case at::ScalarType::Char:
    case at::ScalarType::Short:
      return MakeLtcPrimitiveType(scalar_type, device) != TensorTypeToLtcType(scalar_type);
    default:
      return false;
  }
}
...
lazy_tensors::PrimitiveType GetShapeDimensionType(const Device* device) {
  Device ltc_device = GetDeviceOrCurrent(device);
  return ltc_device.hw_type == DeviceType::CPU ? lazy_tensors::PrimitiveType::S64
                                               : lazy_tensors::PrimitiveType::S32;
}

...
void StridedCopy(D* dest, int64_t dest_stride, const S* source, int64_t source_stride, int64_t n) {
  Caster<S> caster;
...
template <typename S>
int64_t GetFlatTensorOffset(const S& strides, const std::vector<int64_t>& indices) {
  int64_t base = 0;
...
// The at::BFloat16 does not have implicit cast operations, so using
...
template <>
void CopyData<at::BFloat16, at::BFloat16>(at::BFloat16* dest, const at::BFloat16* source, int64_t n,
                                          const CopyCasted&) {
  CheckedMemcpy<at::BFloat16, at::BFloat16>(dest, source, n);
}
...
struct CopyPartition {
  explicit CopyPartition(lazy_tensors::Span<const int64_t> dimensions)
...
std::vector<CopyPartition> CreateCopyPartitions(lazy_tensors::Span<const int64_t> dimensions,
  ...
  int64_t max_parts = std::max<int64_t>(std::thread::hardware_concurrency() / 2, 1);
  ...
  int64_t part_size = std::max<int64_t>(std::max<int64_t>(max_dim_size / max_parts, 1),
...
void SlicedCopy(lazy_tensors::Span<const int64_t> dimensions, const SType* src_data,
                lazy_tensors::Span<const int64_t> src_strides, DType* dest_data,
                lazy_tensors::Span<const int64_t> dest_strides,
                lazy_tensors::Span<const int64_t> iter_dims, const CopyPartition& part) {
...
void TensorToBufferSType(const at::Tensor& tensor, const lazy_tensors::Shape& dest_shape,
  ...
  switch (dest_shape.element_type()) {
    case lazy_tensors::PrimitiveType::BF16:
      TensorToBuffer<SType, at::BFloat16>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                          device);
      break;
    ...
    case lazy_tensors::PrimitiveType::U8:
      TensorToBuffer<SType, uint8_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case lazy_tensors::PrimitiveType::S8:
      TensorToBuffer<SType, int8_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case lazy_tensors::PrimitiveType::S16:
      TensorToBuffer<SType, int16_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case lazy_tensors::PrimitiveType::U16:
      TensorToBuffer<SType, uint16_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case lazy_tensors::PrimitiveType::S32:
      TensorToBuffer<SType, int16_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case lazy_tensors::PrimitiveType::U32:
      TensorToBuffer<SType, uint32_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case lazy_tensors::PrimitiveType::S64:
      TensorToBuffer<SType, int64_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case lazy_tensors::PrimitiveType::U64:
      TensorToBuffer<SType, uint64_t>(tensor, dest_shape, dest_buffer, dest_buffer_size, device);
      break;
    case lazy_tensors::PrimitiveType::C64:
      TensorToBuffer<SType, std::complex<float>>(tensor, dest_shape, dest_buffer, dest_buffer_size,
                                                 device);
      break;
...
at::Tensor MakeTensorFromLiteral(const lazy_tensors::Literal& literal,
  ...
  switch (literal.shape().element_type()) {
    ...
    case lazy_tensors::PrimitiveType::BF16:
      return LiteralToTensorHelper<at::BFloat16>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::F16:
    ...
    case lazy_tensors::PrimitiveType::U64:
      return LiteralToTensorHelper<uint64_t>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::C64:
      return LiteralToTensorHelper<std::complex<float>>(literal, dest_element_type);
    case lazy_tensors::PrimitiveType::C128:
      return LiteralToTensorHelper<std::complex<float>>(literal, dest_element_type);
    default:

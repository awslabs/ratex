...
std::vector<const Node*> Util::ComputePostOrder(lazy_tensors::Span<const Node* const> nodes,
                                                EmissionMap* emap) {
  std::vector<const Node*> post_order;
  for (auto node : nodes) {
    auto node_post_order = ComputePostOrder(node, emap);
    post_order.insert(post_order.end(), node_post_order.begin(), node_post_order.end());
  }
  return post_order;
}
...
std::vector<Value> Util::Clone(lazy_tensors::Span<const Value> values,
  ...
  for (auto node : post_order) {
    ...
    for (auto& output : node->operands()) {
      ...
      LTC_CHECK(it != clone_map.end()) << "Bad post-order: " << node->ToString();

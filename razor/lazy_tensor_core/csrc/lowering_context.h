...
class LoweringContext {
 public:
  ...
  LoweringContext(const std::string& name, Device device,
                  lazy_tensors::Span<const Node* const> post_order, Util::EmissionMap emit_status);

  virtual ~LoweringContext() = default;

  static std::unique_ptr<LoweringContext> Create(const std::string& name, Device device,
  ...
  const std::vector<lazy_tensors::ComputationClient::DataPtr>& GetParametersData() const;
  ...
  virtual lazy_tensors::StatusOr<std::shared_ptr<lazy_tensors::GenericComputation>> Build() = 0;
  ...
  // debugging purposes.
  virtual void AddParameter(const Output& output, size_t index, const lazy_tensors::Shape& shape,

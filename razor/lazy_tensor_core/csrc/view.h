...
struct SelectInfo {
  bool operator==(const SelectInfo& ref) const {
    return dim == ref.dim && start == ref.start && end == ref.end && stride == ref.stride;
  }
...
struct ViewInfo {
  ...
  ViewInfo() = default;
  ViewInfo(Type view_type, lazy_tensors::Shape shape, lazy_tensors::Shape source_shape);
  ViewInfo(Type view_type, lazy_tensors::Shape source_shape, std::vector<int64_t> permutation);
  ViewInfo(Type view_type, const lazy_tensors::Shape& source_shape, SelectInfo select);
  ViewInfo(Type view_type, lazy_tensors::Shape shape, lazy_tensors::Shape source_shape,
           AsStridedInfo as_strided);
  ViewInfo(Type view_type, const lazy_tensors::Shape& source_shape, DiagonalInfo diagonal);

  bool operator==(const ViewInfo& ref) const {
    return view_type == ref.view_type && shape == ref.shape && indices == ref.indices &&
           source_shape == ref.source_shape && permutation == ref.permutation &&
           select == ref.select && as_strided == ref.as_strided && diagonal == ref.diagonal;
  }
...
class Alias {
 public:
  ...
  const ir::Value& ir_value() const {
    return ir_value_;
  }

  const std::vector<UpdateData>& updates() const {
    return updates_;
  }
...
class View {
 public:
  ...
  View(lazy_tensors::Shape shape, std::shared_ptr<Alias> alias, ViewInfo view_info);
  View(lazy_tensors::Shape shape, std::shared_ptr<Alias> alias, std::vector<ViewInfo> view_infos);
  ...
  std::shared_ptr<View> CreateSubView(lazy_tensors::Shape shape, ViewInfo view_info);

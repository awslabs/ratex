...
class BackendImplInterface {
 public:
  virtual std::unique_ptr<NodeLowering> CreateNodeLowering(ir::LoweringContext* loctx) const = 0;
  ...
  virtual std::unique_ptr<ir::LoweringContext> CreateLoweringContext(const std::string& name,
                                                                     Device device) const = 0;

  virtual lazy_tensors::ComputationClient* GetComputationClient() const = 0;

  virtual lazy_tensors::ComputationClient* GetComputationClientIfInitialized() const = 0;

  virtual std::vector<std::string> GetCompilationDevices(
      const std::string& device, lazy_tensors::Span<const std::string> devices) const = 0;

  virtual at::Tensor MakeTensorFromComputationData(
      const lazy_tensors::ComputationClient::DataPtr data,
      c10::optional<at::ScalarType> logical_scalar_type) const = 0;

  virtual lazy_tensors::ComputationClient::DataPtr MakeComputationDataFromTensor(

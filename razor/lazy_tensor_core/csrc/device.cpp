...
void ParseDevice(const std::string& device_spec, Device* device) {
  if (device_spec.empty()) {
    std::string default_device_spec = lazy_tensors::ComputationClient::Get()->GetDefaultDevice();
  ...
  if (device_spec[0] == ':') {
    ...
    return ParseDevice(default_device_spec.substr(0, pos) + device_spec, device);
  }
  std::vector<std::string> device_spec_parts = absl::StrSplit(device_spec, ':');
  LTC_CHECK_EQ(device_spec_parts.size(), 2) << "Invalid device specification: " << device_spec;

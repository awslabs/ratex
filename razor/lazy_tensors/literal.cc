...
const Shape& Literal::shape() const {
  return shape_;
}
...
size_t Literal::Hash() const {
  ...
  ShapeUtil::ForEachSubshape(shape(), [&](const Shape& subshape, const ShapeIndex& index) {
    if (!subshape.IsArray()) {
      return;
    }

    hash_value = Hash64Combine(
        hash_value, Hash64(static_cast<const char*>(untyped_data(index)), size_bytes(index)));
  });

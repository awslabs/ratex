#include "torch_mnm/csrc/serialization/serialization.h"

#include "torch_mnm/csrc/serialization/utils.h"
...
Shape::operator LTCShape() const {
  ...
  return get()->element_type->value == (int)PrimitiveType::TUPLE ?
    ...
    LTCShape(ToLTCFromTVM<PrimitiveType>(get()->element_type),
             dimensions);
}
...
ProgramShape::operator LTCProgramShape() const {
  return LTCProgramShape{
    ToLTCFromTVM<std::vector<LTCShape>>(get()->parameters),
    ToLTCFromTVM<std::vector<std::string>>(
      get()->parameter_names),
...
Computation::operator LTCComputation() const {
  return LTCComputation{
    std::make_shared<LTCGenericComputationMNM>(

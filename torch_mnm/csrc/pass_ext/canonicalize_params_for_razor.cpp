 ...
 * \brief Flat the forward output tuple with mutations (in-place update).
 */
...
class Canonicalizer : public ExprMutator {
 public:
  ...
  Expr operator()(const Expr& e) {
    ...
    auto dy_tuple = bwd_closure->params[0];

    // If the forward output is not a tuple, then do nothing.
    if (!var_to_expr[fwd_out_var].as<TupleNode>() ||
        dy_tuple->checked_type().as<TensorTypeNode>()) {
      return e;
    }
    ...
    Expr body = LetList::With([&](LetList* ll) {
      ...
      // Remove unused parameters in backward closure. They are expected to be
 ...
 private:
  ...
  Var dy_;
  /*! \brief The dy tuple as the backward closure parameter. */
  Var dy_tuple_;
};

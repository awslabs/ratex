...
def compare_tensors(tensor1, tensor2, rtol=1e-05, atol=1e-08, max_diffs=25):
    ...
    for error, i, v1, v2 in top_diffs:
        report += "{}: {} vs. {}\terror={}\n".format(_index_of(sizes1, i), v1, v2, error)
...
def tensor_file_compare(path1, path2, rtol=1e-05, atol=1e-08, max_diffs=25):
    ...
    report = compare_tensors(tensor1, tensor2, rtol=rtol, atol=atol, max_diffs=max_diffs)
    if report:
...
if __name__ == "__main__":
    ...
    report = compare(
        args.savedir1, args.savedir2, rtol=args.rtol, atol=args.atol, max_diffs=args.max_diffs
    )
    if report:

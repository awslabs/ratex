...
def start_server(port: int, only_on_master: Optional[bool] = True) -> object:
    ...
    if not only_on_master or ltm.is_master_ordinal():
        return _TORCHMNMC.profiler.start_server(port)
...
class Trace(_TORCHMNMC.profiler.TraceMe):
    ...
    def __enter__(self):
        self.scope = _TORCHMNMC.profiler.scope_pusher(self.name)
        super().__enter__()

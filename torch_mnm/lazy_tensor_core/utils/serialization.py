...
def save(data, path, master_only=True, global_master=False):
    ...
    Args:
      ...
      global_master (bool, optional): When ``master_only`` is ``True`` this flag
        ...
        Default: False
    """
    should_write_data = not master_only or ltm.is_master_ordinal(local=not global_master)
...
def load(path):
    ...
    def convert_fn(tensors):
        rewritten_tensors = []
        for t in tensors:
            rewritten_tensors.append(torch.load(_get_tensor_file(tensor_folder, t.tid)))
        return rewritten_tensors

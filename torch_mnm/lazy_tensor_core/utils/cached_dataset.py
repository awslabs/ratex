...
import torch_mnm.lazy_tensor_core.utils.gcsfs as gcs
...
class CachedDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data_set, path, max_files_per_folder=1000, compress=True):
        ...
        self._split_count = len(_index_split(self._count, self._max_files_per_folder, 0))

    def _index_path(self, index):
        return os.path.join(
            self._path, *_index_split(index, self._max_files_per_folder, self._split_count)
        )
    ...
    def __getitem__(self, index):
        ...
        if data is None:
            if self._data_set is None:
                raise RuntimeError(
                    "Source dataset not provided and sample {} is missing from cache folder {}".format(
                        index, self._path
                    )
                )

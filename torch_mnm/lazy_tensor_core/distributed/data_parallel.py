...
import _TORCHMNMC
import torch_mnm.lazy_tensor_core.distributed.parallel_loader as pl
...
class DataParallel(object):
    ...
    def _get_model_device(self, model):
        ...
        if len(devices) > 1:
            raise RuntimeError("Model uses more than one device: {}".format(list(devices)))
        return devices.pop() if devices else "cpu"

    def _handle_runner_exception(self, device, e):
        print(
            "Exception in model function for device={}: {}".format(device, str(e)), file=sys.stderr
        )
    ...
    def __call__(self, loop_fn, loader, batchdim=0, **kwargs):
        ...
        if self._native_run:
            ...
            return [
                loop_fn(
                    self._models[0],
                    enumerate(loader),
                    torch.device(self._device_ids[0]),
                    self._contexts[0],
                    self._kwargs,
                )
            ]
        ...
        ltm.wait_device_ops()
        para_loader = pl.ParallelLoader(loader, self._device_ids, batchdim=batchdim, **self._kwargs)
        threads = []
        results = []
        for module, device, context in zip(self._models, self._device_ids, self._contexts):
            result = ThreadResult()

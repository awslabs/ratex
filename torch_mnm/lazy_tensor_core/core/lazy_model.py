...
import _TORCHMNMC
import torch_mnm.lazy_tensor_core.core.ltc_env_vars as xenv
import torch_mnm.lazy_tensor_core.debug.metrics_saver as ms
import torch_mnm.lazy_tensor_core.utils.utils as xu
import torch_mnm.lazy_tensor_core.utils.keyd_queue as kq

_DEVICES = xu.LazyProperty(lambda: _TORCHMNMC._ltc_get_devices())
...
def _get_device_context(device=None):
    if device is None:
        device = _TORCHMNMC._ltc_get_default_device()
    else:
        device = str(device)
...
class CollectiveContext(object):
    def __init__(self, groups=None):
        self.replica_devcount = _TORCHMNMC._ltc_get_replication_devices_count()
...
def _make_group_for_ordinal(ordinal, groups):
    ...
    raise RuntimeError("Ordinal {} not found in groups {}".format(ordinal, groups))
...
def lazy_device(n=None, devkind=None):
    ...
    """
    if n is None:
        devices = get_lazy_supported_devices(devkind=devkind if devkind is not None else None)
    ...
    _TORCHMNMC._ltc_set_default_device(device)
...
def lazy_replication_devices(local_devices):
    ...
    if len(kind_devices) != len(local_devices):
        ...
        raise RuntimeError(
            "Cannot replicate if number of devices ({}) is different from {}".format(
                len(local_devices), len(kind_devices)
            )
        )
    replication_devices = []
    for device in _TORCHMNMC._ltc_get_all_devices():
        xdev = parse_lazy_device(device)
        if not xdev:
            raise RuntimeError("Invalid device format: {}".format(device))
        if xdev[0] == device_type:
            replication_devices.append(device)
    sorted_by_ordinal = sorted(replication_devices, key=lambda device: parse_lazy_device(device)[1])
...
def unlazy(tensors):
    ...
    """
    _TORCHMNMC._ltc_sync_multi(tensors, devices=[], wait=True)
...
def set_replication(device, devices):
    ...
    if devices:
        ...
        _TORCHMNMC._ltc_set_replication_devices(replication_devices)
        devctx.device_index = devices.index(device)
    else:
        _TORCHMNMC._ltc_set_replication_devices([])
...
class RateTracker(object):
    def __init__(self, smooth_factor=None):
        self._smooth_factor = (
            xu.getenv_as("RATE_TRACKER_SMOOTHING", float, 0.4)
...
class ToLazyTensorArena(object):
    ...
    def _replace_tensors(self, inputs):
        ...
        return xu.for_each_instance_rewrite(inputs, lambda x: self._select_fn(x), convert_fn)
...
def check_view_sharing(obj):
    ...
    def tensor_id(t):
        if is_lazy_tensor(t):
            return _TORCHMNMC._ltc_get_tensor_id(t), "lazy"
        return id(t), "torch"

    def alias_id(t):
        if is_lazy_tensor(t):
            aid = _TORCHMNMC._ltc_get_tensor_view_alias_id(t)
                ...
                if aid in aliases:
                    ...
                    raise RuntimeError(
                        "Tensor ID {} ({}) is sharing a view with tensor ID {} ({})".format(
                            tid, tensor_info(obj), tensor_id(oobj), tensor_info(oobj)
                        )
                    )
                aliases[aid] = obj
...
def _get_all_reduce_token():
    ...
    if token is None:
        token = _TORCHMNMC._ltc_create_token(devctx.device)
...
def _torch_all_reduce(reduce_type, inputs, group=None):
    ...
    for tensor in inputs:
        ...
        cpu_tensor = _TORCHMNMC._ltc_get_cpu_tensors([tensor])[0]
        results.append(cpu_tensor)
        if async_op is not None:
            async_op.wait()
        async_op = dist.all_reduce(cpu_tensor, reduce_op, async_op=True, group=group)
    if async_op is not None:
        async_op.wait()
    return results
...
def _host_all_reduce(reduce_type, inputs, cctx, scale=None):
    ...
    _TORCHMNMC._ltc_sync_multi(inputs, devices=[], wait=True, sync_ltc_data=True)
    ...
    if cctx.requires_intercore_reduce:
        ...
        devctx.all_reduce_token = _TORCHMNMC._ltc_all_reduce_inplace(
...
def all_reduce(reduce_type, inputs, scale=1.0, groups=None, cctx=None):
    ...
    if cctx.requires_intercore_reduce:
        ...
        if isinstance(inputs, torch.Tensor):
            result = _TORCHMNMC._ltc_all_reduce(
...
def all_to_all(value, split_dimension, concat_dimension, split_count, groups=None):
    ...
    """
    token, devctx = _get_all_reduce_token()
    result = _TORCHMNMC._ltc_all_to_all(
        value, token, split_dimension, concat_dimension, split_count, groups or []
    )
    devctx.all_reduce_token = result[1]
    return result[0]
...
def collective_permute(value, pairs):
    ...
    """
    token, devctx = _get_all_reduce_token()
    result = _TORCHMNMC._ltc_collective_permute(value, token, pairs)
    devctx.all_reduce_token = result[1]
    return result[0]
...
def mark_step():
    ...
    _TORCHMNMC._ltc_step_marker(
        _TORCHMNMC._ltc_get_default_device(),
        [],
...
def wait_device_ops(devices=[]):
    ...
    """
    _TORCHMNMC._ltc_wait_device_ops(devices=devices)
...
def reduce_gradients(optimizer, groups=None):
    ...
    if count > 1:
        ...
        all_reduce(REDUCE_SUM, gradients, scale=1.0 / count, groups=groups, cctx=cctx)
...
def _maybe_convert_to_cpu(data, convert=True):
    def convert_fn(tensors):
        _TORCHMNMC._ltc_sync_multi(tensors, devices=[], wait=True, sync_ltc_data=True)
        if not convert:
            return tensors
        return _TORCHMNMC._ltc_get_cpu_tensors(tensors)
...
def send_cpu_data_to_device(data, device):
    def convert_fn(tensors):
        ...
        return _TORCHMNMC._ltc_tensors_from_aten(tensors, devices)
...
def rendezvous(tag, payload=b"", replicas=[]):
    ...
    """
    return _TORCHMNMC._lazy_rendezvous(get_ordinal(), tag, payload, replicas)
...
def set_rng_state(seed, device=None):
    ...
    if device is None:
        device = _TORCHMNMC._ltc_get_default_device()
    _TORCHMNMC._ltc_set_rng_seed(seed, str(device) if device else "")
...
def get_rng_state(device=None):
    ...
    if device is None:
        device = _TORCHMNMC._ltc_get_default_device()
    return _TORCHMNMC._ltc_get_rng_seed(str(device) if device else "")
...
def get_memory_info(device):
    ...
    Returns:
      ...
      memory in KB) keys.
    """
    return _TORCHMNMC._ltc_memory_info(str(device))

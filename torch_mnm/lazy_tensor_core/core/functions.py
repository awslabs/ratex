...
class AllReduce(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.reduce_type == ltm.REDUCE_MUL:
            ...
            grad_scaler = torch.where(input != 0, output / input, torch.zeros_like(input))
            return grad * grad_scaler, None, None, None
        if ctx.reduce_type == ltm.REDUCE_MIN or ctx.reduce_type == ltm.REDUCE_MAX:
            return torch.where(input == output, grad, torch.zeros_like(grad)), None, None, None
...
class AllGather(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        return (
            torch.narrow(grad_output.clone(), ctx.dim, ctx.ordinal * slice_size, slice_size),
            None,
        )
...
def nms(boxes, scores, score_threshold, iou_threshold, output_size):
    ...
    """
    return _TORCHMNMC._ltc_nms(boxes, scores, score_threshold, iou_threshold, output_size)

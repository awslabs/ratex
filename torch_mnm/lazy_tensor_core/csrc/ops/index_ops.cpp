...
void CheckIndexTensorTypes(const c10::List<c10::optional<at::Tensor>>& indices) {
  for (const c10::optional<at::Tensor>& tensor : indices) {
    if (tensor.has_value() && tensor->defined()) {
      at::ScalarType scalar_type = tensor->scalar_type();
      if (scalar_type != at::kLong && scalar_type != at::kByte && scalar_type != at::kBool) {
...
std::vector<at::Tensor> ExpandByteTensors(const at::Tensor& self,
  ...
  for (const c10::optional<at::Tensor>& index : indices) {
    if (index.has_value() &&
        (index->scalar_type() == at::kByte || index->scalar_type() == at::kBool)) {
      ...
      for (int64_t j = 0; j < index->dim(); j++) {
        ...
        LTC_CHECK_EQ(index->size(j), self.size(src_idx))
            ...
            << " does not match the shape of the indexed tensor " << self.sizes() << " at index "
            << src_idx;
      }
...
CanonicalIndexInfo TransposeToFront(at::Tensor base, at::TensorList indices) {
  ...
  if (adjacency_info.contiguous_non_null) {
    return {base, std::move(transposed_indices),
            lazy_tensors::util::Iota<lazy_tensors::int64>(base_rank), adjacency_info.start_dim};
  }
  ...
  for (size_t dim_idx = 0; dim_idx < indices.size(); ++dim_idx) {
    ...
    LazyTensor wrapped_dim_index = LazyTensor::Create(
        dim_index.GetIrValue() +
            LazyTensor::GetIrValueForScalar(dim_size, dim_index.shape(), base.GetDevice()),
        base.GetDevice());
    LazyTensor wrap_cond = LazyTensor::lt(indices[dim_idx], at::Scalar(int64_t(0)));
    canonical_indices.push_back(LazyTensor::where(wrap_cond, wrapped_dim_index, dim_index));
  }
  return canonical_indices;
}

ir::NodePtr IndexFillOp(const ir::Value& buffer, lazy_tensors::int64 dim, const ir::Value& index,
                        const ir::Value& value) {
  ir::Value index_rank1 = EnsureRank1(index);
  ir::NodePtr node = ir::MakeNode<ir::ops::IndexAlongDim>(ir::OpKind(at::aten::index_fill), buffer,
                                                          index_rank1, value, dim);
  node->SetShapeDeferred([&]() { return compiler::NodeLowering::Get()->Infer(node.get()); });
  return node;
}

ir::NodePtr IndexAddOp(const ir::Value& buffer, lazy_tensors::int64 dim, const ir::Value& index,
                       const ir::Value& source) {
  ir::Value index_rank1 = EnsureRank1(index);
  ir::NodePtr node = ir::MakeNode<ir::ops::IndexAlongDim>(ir::OpKind(at::aten::index_add), buffer,
                                                          index_rank1, source, dim);
  node->SetShapeDeferred([&]() { return compiler::NodeLowering::Get()->Infer(node.get()); });
  return node;
}

ir::NodePtr IndexCopyOp(const ir::Value& buffer, lazy_tensors::int64 dim, const ir::Value& index,
                        const ir::Value& source) {
  ir::Value index_rank1 = EnsureRank1(index);
  ir::NodePtr node = ir::MakeNode<ir::ops::IndexAlongDim>(ir::OpKind(at::aten::index_copy), buffer,
                                                          index_rank1, source, dim);
  node->SetShapeDeferred([&]() { return compiler::NodeLowering::Get()->Infer(node.get()); });
  return node;
}
...
CanonicalIndexInfo GetCanonicalIndexInfo(const at::Tensor& base,
  ...
  for (size_t i = 0; i < canonical_index_info.indices.size(); i++) {
    if (canonical_index_info.indices[i].device() != base.device()) {
      canonical_index_info.indices[i] = canonical_index_info.indices[i].to(base.device());
    }
  }
...
ir::Value EnsureRank1(const ir::Value& index) {
  ...
  return index->shape().rank() == 0
             ? ir::MakeNode<ir::ops::Expand>(index, std::vector<lazy_tensors::int64>{1})
             : index;
}
...
LazyTensor IndexByTensors(const LazyTensor& base, lazy_tensors::Span<const LazyTensor> indices,
  ...
  lazy_tensors::int64 indices_rank = canonical_indices.front().shape().get().rank();
  ...
  return LazyTensor::Create(
      ir::MakeNode<ir::ops::IndexGet>(base.GetIrValue(), indices_nd.GetIrValue(), start_dim),
...
ir::Value IndexPutByTensors(const LazyTensor& base, lazy_tensors::Span<const LazyTensor> indices,
  ...
  return ir::MakeNode<ir::ops::Permute>(
      ir::MakeNode<ir::ops::IndexPut>(base.GetIrValue(), indices_nd.GetIrValue(), start_dim,
...
ir::NodePtr IndexFill(const LazyTensor& base, lazy_tensors::int64 dim, const LazyTensor& index,
  ...
  LTC_CHECK_EQ(index.dtype(), at::ScalarType::Long)
      << "Fill index is expected to be of scalar type Long, but it is " << index.dtype();
  LTC_CHECK_LE(index.shape().get().rank(), 1) << "Fill index is supposed to be a vector";
  return IndexFillOp(
      base.GetIrValue(), dim, index.GetIrValue(),
      LazyTensor::GetIrValueForScalar(value, base.shape().get().element_type(), base.GetDevice()));
}
...
ir::NodePtr IndexFill(const LazyTensor& base, lazy_tensors::int64 dim, const LazyTensor& index,
  ...
  LTC_CHECK_EQ(index.dtype(), at::ScalarType::Long)
      << "Fill index is expected to be of scalar type Long, but it is " << index.dtype();
  LTC_CHECK_LE(index.shape().get().rank(), 1) << "Fill index is supposed to be a vector";
  LTC_CHECK_EQ(value.shape().get().rank(), 0) << "Fill only supports a 0-dimensional value tensor";
  return IndexFillOp(base.GetIrValue(), dim, index.GetIrValue(), value.GetIrValue());
}

ir::Value IndexAdd(const LazyTensor& base, lazy_tensors::int64 dim, const LazyTensor& index,
                   const LazyTensor& source) {
  LTC_CHECK(index.dtype() == at::ScalarType::Long || index.dtype() == at::ScalarType::Int)
      << "Add index is expected to be of scalar type Long or scalar type Int, "
         "but it is "
      << index.dtype();
  LTC_CHECK_LE(index.shape().get().rank(), 1) << "Add index is supposed to be a vector";
  return IndexAddOp(base.GetIrValue(), dim, index.GetIrValue(), source.GetIrValue());
}

ir::Value IndexCopy(const LazyTensor& base, lazy_tensors::int64 dim, const LazyTensor& index,
                    const LazyTensor& source) {
  LTC_CHECK_EQ(index.dtype(), at::ScalarType::Long)
      << "Copy index is expected to be of scalar type Long, but it is " << index.dtype();
  LTC_CHECK_LE(index.shape().get().rank(), 1) << "Copy index is supposed to be a vector";
  return IndexCopyOp(base.GetIrValue(), dim, index.GetIrValue(), source.GetIrValue());
}

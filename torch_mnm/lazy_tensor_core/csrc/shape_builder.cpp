...
ShapeBuilder& ShapeBuilder::Add(const lazy_tensors::Shape& shape, lazy_tensors::int64 dim) {
...
lazy_tensors::Shape ShapeBuilder::Build() const {
  ...
  lazy_tensors::Shape shape = lazy_tensors::ShapeUtil::MakeShape(type_, dimensions);
  for (lazy_tensors::int64 i = 0; i < shape.rank(); ++i) {
    const ShapeDim& sdim = dims_[i];
    if (sdim.shape != nullptr) {
      shape.set_dynamic_dimension(i, sdim.shape->is_dynamic_dimension(sdim.dim_or_size));
    }
  }
  return shape;
}

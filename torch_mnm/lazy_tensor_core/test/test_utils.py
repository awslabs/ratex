...
import torch_mnm.lazy_tensor_core.core.lazy_model as ltm
import torch_mnm.lazy_tensor_core.debug.metrics as met
import torch_mnm.lazy_tensor_core.debug.metrics_compare_utils as mcu
...
def mp_test(func):
    ...
    Example::
      ...
      import torch_mnm.lazy_tensor_core.test.test_utils as xtu
      import unittest
...
def write_to_summary(summary_writer, global_step=None, dict_to_write={}, write_ltc_metrics=False):
...
def get_summary_writer(logdir):
    ...
    if logdir:
        ...
        write_to_summary(writer, 0, dict_to_write={"TensorboardStartTimestamp": time.time()})
        return writer
...
def print_training_update(device, step, loss, rate, global_rate, epoch=None, summary_writer=None):

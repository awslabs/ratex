...
if [[ ${INPLACE_FORMAT} -eq 1 ]]; then
    ...
    git-${CLANG_FORMAT_BIN} --extensions h,mm,c,cc,cpp --binary=${CLANG_FORMAT_BIN} $1
    exit 0
fi

echo "Running git-clang-format against" $1
git-${CLANG_FORMAT_BIN} --diff --extensions h,mm,c,cc,cpp --binary=${CLANG_FORMAT_BIN} $1 1> /tmp/$$.clang-format.txt
